<template>
    <div>
        <div dir='rtl' class="bg-purple-900 rounded-md text-white px-4 pt-10 pb-5 m-auto mt-12 w-11/12 load-component" style="hover:" :style="'animation-delay: '+3*(loaded_component++/10 - 0.1)+'s'">
            <div class="bg-purple-700 rounded-md p-3 w-8/12 m-auto flex gap-2 justify-center text-lg hover:scale-105 shadow cursor-pointer items-center">
                <IconBanknote class="w-8 h-8 -mr-1"/>
                <p class="samim-font samim-fd-font">{{ nFormat.format(balance) }}</p>
                <p class="samim-font">تومان</p> 
            </div>

            <div class="bg-orange-700 px-3 py-3 rounded-md flex justify-center w-6/12 m-auto mt-12 cursor-pointer hover:bg-orange-600 gap-2 items-center hover:box-shadow-bottom-button-buy hover:scale-105">
                <IconDeposit class="w-7 h-7 mt-1 -mr-3"/>
                <p class="samim-font">افزایش موجودی</p>
            </div>
            
            <div class="bg-purple-700 rounded-md w-7/12 px-3 py-4  flex justify-center m-auto hover:scale-105 mt-3 gap-1 hover:bg-purple-800 shadow" @click="inviteFriend">
        ...
        <div dir='rtl' class="bg-purple-900 rounded-md text-white p-5 m-auto mt-8 w-11/12 load-component mb-20" :style="'animation-delay: '+3*(loaded_component++/10 - 0.1)+'s'">
            ...
            <form class="mt-4 w-full flex" @submit.prevent="submit">
                <input type="text" dir="ltr" class="rounded-r-md px-2 py-3 w-8/12 shadow outline-none bg-purple-500">
                <button class="bg-orange-700 px-2 py-3 samim-font rounded-l-md hover:bg-orange-600 hover:scale-105 hover:box-shadow-bottom-button-buy outline-none">دریافت هدیه</button>
            </form>
        </div>
    </div>
</template>
